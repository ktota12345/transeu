...
@Controller('vehicle-equipment')
@UseGuards(JwtAuthGuard)
export class VehicleEquipmentController {
    constructor(private readonly service: VehicleEquipmentService) {}

    @Post()
    create(@Body() dto: Prisma.VehicleEquipmentCreateInput) {
    ...
    async list(
        ...
        res.setHeader('Content-Range', `vehicle-equipment ${skip}-${skip + items.length - 1}/${total}`);
    ...
    @Put(':id')
    update(@Param('id') id: string, @Body() dto: Prisma.VehicleEquipmentUpdateInput) {
